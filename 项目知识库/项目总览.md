...
- `tools/ops/` — 运维/诊断脚本；`tools/security/`、`tools/wq_crawl/`
- `docs/` — 现行文档含冻结方案
- `文档/历史资料/` — 历史资料
- `legacy/` — 已清空观察区说明（活跃遗留在 `alpha_mining/legacy/`）
- `World quant/` / `数据/` / `项目知识库/` / `配置/`
- 根保留回归单体：`auto_alpha_pipeline_rebuilt_v50.py`
## 主知识库摘录
